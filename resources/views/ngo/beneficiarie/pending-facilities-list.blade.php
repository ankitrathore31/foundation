                            ...
                            @foreach ($beneficiarie as $item)
                                @foreach ($item->facilities as $facility)
                                    <tr>
                                        ...
                                        <td>
                                            {{ $facility->distribute_date ? \Carbon\Carbon::parse($facility->distribute_date)->format('d-m-Y') : 'Not Found' }}
                                        </td>
                                        <td>{{ $item->distribute_place ?? 'Not Found' }}</td>
                                        <td>{{ $facility->facilities_category ?? 'Not Found' }}</td>
                                        <td>{{ $facility->facilities ?? 'Not Found' }}</td>
                                        <td>{{ $facility->status ?? 'Not Found' }}</td>
                                        <td>{{ $facility->pending_reason ?? 'Not Found' }}</td>
                                        <td></td>
                                        <td>{{ $facility->bene_category ?? 'Not Found' }}</td>
                                        <td>{{ $facility->academic_session ?? 'Not Found' }}</td>
                                        <td class="no-print">
                                            <div class="d-flex justify-content-center gap-2 flex-wrap">
                                                <a href="{{ route('show-beneficiarie-report', [$item->id, $facility->id]) }}"
                                                    class="btn btn-success btn-sm px-3 d-flex align-items-center justify-content-center"
                                                    title="View">
                                                    <i class="fa-regular fa-eye"></i>
                                                </a>
                                                <a href="{{ route('edit-distribute-facilities', [$item->id, $facility->id]) }}"
                                                    class="btn btn-success btn-sm px-3 d-flex align-items-center justify-content-center"
                                                    title="Edit">
