...
@Section('content')
    ...
    <div class="wrapper">
        <div class="container-fluid mt-4">
            ...
            <div class="row">
                <div class="col-md-12">
                    <form method="GET" action="{{ route('beneficiarie-facilities-list') }}" class="row mb-4">
                        <div class="col-md-4">
                        ...
                        <tbody>
                            ...
                            @foreach ($beneficiarie as $item)
                                @foreach ($item->surveys as $survey)
                                    @forelse ($survey->facilities as $facility)
                                        <tr>
                                            <td>{{ $serial++ }}</td>
                                            ...
                                            <td>
                                                {{ $survey->survey_date ? \Carbon\Carbon::parse($survey->survey_date)->format('d-m-Y') : 'Not Found' }}
                                            </td>
                                            <td>{{ $facility->facilities_category ?? 'No Found' }}</td>
                                            <td>{{ $facility->facilities ?? 'No Found' }}</td>
                                            <td>{{ $survey->bene_category ?? 'No Found' }}</td>
                                            <td>{{ $survey->academic_session }}</td>
                                            <td class="no-print">
                                                <div class="d-flex justify-content-center gap-2 flex-wrap">
                                                    <a href="{{ route('distribute-beneficiarie-facilities', ['beneficiarie_id' => $item->id, 'facility_id' => $facility->id]) }}"
                                                    ...
                                                    <a href="{{ route('show-beneficiarie-facilities', [$item->id, $survey->id, $facility->id]) }}"
                                                        class="btn btn-success btn-sm px-3 d-flex align-items-center justify-content-center"
                                                        title="View" style="min-width: 38px; height: auto;">
                                                        <i class="fa-regular fa-eye"></i> Facilities
                                                    </a>

                                                    <a href="{{ route('edit-facilities', [$item->id, $facility->id]) }}"
                                                        class="btn btn-primary btn-sm px-3 d-flex align-items-center justify-content-center"
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="18" class="text-center">No Facilities Found</td>
                                        </tr>
                                    @endforelse
                                @endforeach
                            @endforeach
