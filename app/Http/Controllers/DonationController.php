...
class DonationController extends Controller
{
    ...
    {
        $online = OnlineDonation::where('status', 'Successful');
    ...
    public function DonationReport(Request $request)
    {
        ...
        $onlineQuery = OnlineDonation::where('status', 'Successful');
        ...
        $totalOnline = OnlineDonation::where('status', 'Successful')->sum('amount');
        $totalDonation = $totalOffline + $totalOnline;
        $thisYear = Donation::whereYear('date', now()->year)->sum('amount')
            + OnlineDonation::where('status', 'Successful')->whereYear('date', now()->year)->sum('amount');

        $thisMonth = Donation::whereYear('date', now()->year)
            ->whereMonth('date', now()->month)->sum('amount')
            + OnlineDonation::where('status', 'Successful')
